 ...
 *
 *=========================================================================*/
#ifndef __itkMattesMutualInformationSingleThreadedImageToImageMetricv4GetValueAndDerivativeThreader_hxx
#define __itkMattesMutualInformationSingleThreadedImageToImageMetricv4GetValueAndDerivativeThreader_hxx
...
void
MattesMutualInformationSingleThreadedImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric,
...
::BeforeThreadedExecution()
{
  ...
  this->m_MattesAssociate->m_ThreaderFixedImageMarginalPDF.resize(this->m_MattesAssociate->m_NumberOfHistogramBins,0.0F);
...
template< typename TDomainPartitioner, typename TImageToImageMetric, typename TMattesMutualInformationMetric >
bool
MattesMutualInformationSingleThreadedImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TMattesMutualInformationMetric >
                ...
                const ThreadIdType) const
{
  ...
  this->m_MattesAssociate->m_ThreaderFixedImageMarginalPDF[fixedImageParzenWindowIndex] += 1;
  ...
  JointPDFValueType *pdfPtr = this->m_MattesAssociate->m_ThreaderJointPDF->GetBufferPointer()
  ...
  if( doComputeDerivative && ( this->m_MattesAssociate->HasLocalSupport() ) )
    ...
    localDerivativeOffset =
      this->m_MattesAssociate->ComputeParameterOffsetFromVirtualIndex( virtualIndex,
  ...
  JacobianReferenceType jacobian = this->m_GetValueAndDerivativePerThreadVariables[0].MovingTransformJacobian;
  if( doComputeDerivative )
    {
    JacobianReferenceType jacobianPositional = this->m_GetValueAndDerivativePerThreadVariables[0].MovingTransformJacobianPositional;
  ...
  const bool transformIsDisplacement =
    this->m_MattesAssociate->m_MovingTransform->GetTransformCategory() == MovingTransformType::DisplacementField;
    ...
    *( pdfPtr++ ) += val;
    ...
    if( doComputeDerivative )
      ...
      if( transformIsDisplacement )
        ...
        {
        // Compute PDF derivative contribution.
        this->ComputePDFDerivativesGlobalSupportTransform(fixedImageParzenWindowIndex,
                                                          jacobian,
                                                          pdfMovingIndex,
                                                          movingImageGradient,
                                                          cubicBSplineDerivativeValue);
  ...
  this->m_GetValueAndDerivativePerThreadVariables[0].NumberOfValidPoints++;
...
void
MattesMutualInformationSingleThreadedImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TMattesMutualInformationMetric >
::ComputePDFDerivativesGlobalSupportTransform(const OffsetValueType &         fixedImageParzenWindowIndex,
                                              ...
                                              const PDFValueType &            cubicBSplineDerivativeValue) const
{
  ...
  JointPDFDerivativesValueType *derivPtr = this->m_MattesAssociate->m_ThreaderJointPDFDerivatives->GetBufferPointer()
    + ( pdfFixedIndex  * this->m_MattesAssociate->m_ThreaderJointPDFDerivatives->GetOffsetTable()[2] )
    + ( pdfMovingIndex * this->m_MattesAssociate->m_ThreaderJointPDFDerivatives->GetOffsetTable()[1] );
