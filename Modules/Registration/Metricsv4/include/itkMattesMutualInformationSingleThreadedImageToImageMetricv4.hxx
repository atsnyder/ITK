 ...
 *
 *=========================================================================*/
#ifndef __itkMattesMutualInformationSingleThreadedImageToImageMetricv4_hxx
#define __itkMattesMutualInformationSingleThreadedImageToImageMetricv4_hxx

#include "itkMattesMutualInformationSingleThreadedImageToImageMetricv4.h"
...
MattesMutualInformationSingleThreadedImageToImageMetricv4<TFixedImage,
                                                          TMovingImage,
                                                          TVirtualImage,
                                                          TInternalComputationValueType,
                                                          TMetricTraits>
::MattesMutualInformationSingleThreadedImageToImageMetricv4() :
...
::SetMaximumNumberOfThreads(const ThreadIdType threads)
{
  if(threads > 1)
    {
    itkExceptionMacro(<< "Number of threads requested > 1 " << threads);
    }
  this->Superclass::SetMaximumNumberOfThreads(threads);
}
...
MattesMutualInformationSingleThreadedImageToImageMetricv4<TFixedImage,
                                                          ...
                                                          TInternalComputationValueType,
                                                          TMetricTraits>
::InitializeThread( const ThreadIdType )
{
  /* This block of code is from
     MattesMutualImageToImageMetric::GetValueAndDerivativeThreadPreProcess */
  std::fill(this->m_ThreaderFixedImageMarginalPDF.begin(),
            this->m_ThreaderFixedImageMarginalPDF.end(), 0.0F);
  ...
  const bool reinitializeThreaderJointPDF =
    ( this->m_ThreaderJointPDF.IsNull() ||
      (jointPDFRegion != this->m_ThreaderJointPDF->GetBufferedRegion()) );
  if (  reinitializeThreaderJointPDF )
    ...
    {
    this->m_ThreaderJointPDF = JointPDFType::New();
    this->m_ThreaderJointPDF->SetRegions(jointPDFRegion);
    this->m_ThreaderJointPDF->SetOrigin(origin);
    this->m_ThreaderJointPDF->SetSpacing(spacing);
    // NOTE: true = initialize to zero
    this->m_ThreaderJointPDF->Allocate(true);
    }
    }
  ...
  if( this->GetComputeDerivative()  &&  ! this->HasLocalSupport() )
    ...
    const bool reinitializeThreaderJointPDFDerivatives = ( this->m_ThreaderJointPDFDerivatives.IsNull() ||
                                                           (jointPDFDerivativesRegion != this->m_ThreaderJointPDFDerivatives->GetBufferedRegion() ));
    if( reinitializeThreaderJointPDFDerivatives )
      {
      // Set the regions and allocate
      this->m_ThreaderJointPDFDerivatives = JointPDFDerivativesType::New();
      this->m_ThreaderJointPDFDerivatives->SetRegions( jointPDFDerivativesRegion);
      this->m_ThreaderJointPDFDerivatives->Allocate(true);
      }
    }
}
...
MattesMutualInformationSingleThreadedImageToImageMetricv4<TFixedImage,
                                                          ...
                                                          TInternalComputationValueType,
                                                          TMetricTraits>
::FinalizeThread( const ThreadIdType )
{
  ...
  const IndexValueType numberOfPDFSubsections = 1;

  const IndexValueType numberOfDerivativesSubsections = 1;

  bool isPDFSectionDone(false);
  bool isPDFDerivativeSectionDone(false);
  ...
  JointPDFValueType * const threadPdfStart = this->m_ThreaderJointPDF->GetBufferPointer();
  ...
  if( needDerivativesComputation )
    ...
    JointPDFDerivativesValueType  * const threadPdfDPtrStart = this->m_ThreaderJointPDFDerivatives->GetBufferPointer();
    ...
    for( IndexValueType subsection = 0; subsection < numberOfDerivativesSubsections; ++subsection )
      ...
      if( ! isPDFDerivativeSectionDone )
        ...
        isPDFDerivativeSectionDone = true;
        }
      }
    }
...
::ComputeResults() const
{
  ...
  for( unsigned int i = 0; i < this->m_NumberOfHistogramBins; ++i )
    {
    totalMassOfPDF += this->m_ThreaderFixedImageMarginalPDF[i];
    }
  ...
  for( unsigned int bin = 0; bin < this->m_NumberOfHistogramBins; ++bin )
    {
    this->m_ThreaderFixedImageMarginalPDF[bin] /= totalMassOfPDF;
    }
  ...
  for( unsigned int fixedIndex = 0; fixedIndex < this->m_NumberOfHistogramBins; ++fixedIndex )
    {
    const PDFValueType fixedImagePDFValue = this->m_ThreaderFixedImageMarginalPDF[fixedIndex];
