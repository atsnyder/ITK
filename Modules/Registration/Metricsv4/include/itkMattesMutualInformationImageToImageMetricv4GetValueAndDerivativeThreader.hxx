...
::BeforeSingleExecution()
{
  ...
  this->m_MattesAssociate->m_SingleFixedImageMarginalPDF = std::vector<PDFValueType>(this->m_MattesAssociate->m_NumberOfHistogramBins,0.0F);
 ...
 // std::fill(
   // this->m_MattesAssociate->m_SingleFixedImageMarginalPDF.begin(),
   // this->m_MattesAssociate->m_SingleFixedImageMarginalPDF.end(), 0.0F);
...
bool
MattesMutualInformationImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TMattesMutualInformationMetric >
::SingleProcessPoint( const VirtualIndexType &           virtualIndex,
                const VirtualPointType &           virtualPoint,
                const FixedImagePointType &,
                const FixedImagePixelType &        fixedImageValue,
                const FixedImageGradientType &,
                ...
                DerivativeType & ) const
{
  ...
  // the fixed image marginal pdf, we need only increment the
  // fixedImageParzenWindowIndex by value of 1.0.
  this->m_MattesAssociate->m_SingleFixedImageMarginalPDF[fixedImageParzenWindowIndex] += 1;
  ...
  // Pointer to affected bin to be updated
  JointPDFValueType *pdfPtr = this->m_MattesAssociate->m_AccumulatorJointPDF->GetBufferPointer()
                              + ( fixedImageParzenWindowIndex * this->m_MattesAssociate->m_NumberOfHistogramBins ) + pdfMovingIndex;
  ...
  // Compute the transform Jacobian.
  typedef JacobianType & JacobianReferenceType;
  JacobianReferenceType jacobian = this->m_GetValueAndDerivativeSingleVariables->MovingTransformJacobian;
  if( doComputeDerivative )
    {
    JacobianReferenceType jacobianPositional = this->m_GetValueAndDerivativeSingleVariables->MovingTransformJacobianPositional;
    this->m_MattesAssociate->GetMovingTransform()->
      ComputeJacobianWithRespectToParametersCachedTemporaries(virtualPoint,
                                                              jacobian,
                                                              jacobianPositional);
    ...
    if( doComputeDerivative )
      ...
      if( transformIsDisplacement )
       ...
       /* this->ComputePDFDerivativesGlobalSupportTransform(threadId,
          ...
          movingImageGradient,
          cubicBSplineDerivativeValue);*/
        this->SingleComputePDFDerivativesGlobalSupportTransform(fixedImageParzenWindowIndex,
          jacobian,
          pdfMovingIndex,
          movingImageGradient,
          cubicBSplineDerivativeValue);
...
void
MattesMutualInformationImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TMattesMutualInformationMetric >
::SingleComputePDFDerivativesGlobalSupportTransform(const OffsetValueType &         fixedImageParzenWindowIndex,
                        const JacobianType &            jacobian,
                        const OffsetValueType &         pdfMovingIndex,
                        const MovingImageGradientType & movingImageGradient,
                        const PDFValueType &            cubicBSplineDerivativeValue) const
{
  ...
  JointPDFDerivativesValueType *derivPtr = this->m_MattesAssociate->m_AccumulatorJointPDFDerivatives->GetBufferPointer()
      + ( pdfFixedIndex  * this->m_MattesAssociate->m_AccumulatorJointPDFDerivatives->GetOffsetTable()[2] )
      + ( pdfMovingIndex * this->m_MattesAssociate->m_AccumulatorJointPDFDerivatives->GetOffsetTable()[1] );
...
::AfterSingleExecution()
{
  ...
  this->m_MattesAssociate->m_NumberOfValidPoints += this->m_GetValueAndDerivativeSingleVariables->NumberOfValidPoints;
  ...
  this->m_MattesAssociate->GetValueCommonAfterSingleExecution();

  // Collect and compute results.
  // Value and derivative are stored in member vars.
  this->m_MattesAssociate->SingleComputeResults();
}
