...
class ProcessClassFromFile:
    def __init__(self, inputHeader, minimumRepeatNumber,inputSymbolsFile='/scratch/kent/itkexplicit/SymbolCache' ):
...
"""
    ...
    def _ParseSymbolFile(self,inputSymbolsFile):
        ...
        nm_link_objects = re.compile("itk::"+self.BaseClassName+"<.*>")
        with open(inputSymbolsFile,'r') as isf:
            symbolCache = pickle.load(isf)
        matchingKeys = [x for x in symbolCache.keys() if re.match(nm_link_objects,x) ]
        for key in matchingKeys:
            ...
            for subsPat in self.subsPatterns:
                key = key.replace("std::__1::","std::")
                key = key.replace(subsPat[0],subsPat[1])
            # Remove items from exclude matches
            for excludePat in self.excludePatterns:
                if key.find(excludePat) != -1:
                    key = ""
            self.SymbolsMapping[key] = symbolCache[key];
